...
fn test_generate_watch_scripts_creates_files() {
  with_io_dir(|tmp_path, io| {
    ...
    let deps = vec!["user/lib1".to_string()];
    generate_watch_scripts(tmp_path, &repos_path, &deps, io, make_flags()).unwrap();
...
fn test_generate_watch_scripts_falls_back_to_build() {
  with_io_dir(|tmp_path, io| {
    let deps_path = tmp_path.join("repos");
    make_repo_with_scripts(&deps_path, "user-lib1", r#""build": "tsdown""#);
    let deps = vec!["user/lib1".to_string()];
    generate_watch_scripts(tmp_path, &deps_path, &deps, io, make_flags()).unwrap();
    let ps1 = read_to_string(tmp_path.join("watch.ps1")).unwrap();
...
fn test_generate_watch_scripts_no_scripts_field() {
  with_io_dir(|tmp_path, io| {
    let deps_path = tmp_path.join("repos");
    create_dir_all(deps_path.join("user-lib1")).unwrap();
    write(
      deps_path.join("user-lib1").join("package.json"),
      r#"{"name": "@user/lib1"}"#,
    )
    .unwrap();
    let deps: Vec<String> = vec!["user/lib1".to_string()];
    generate_watch_scripts(tmp_path, &deps_path, &deps, io, make_flags()).unwrap();
    let ps1 = read_to_string(tmp_path.join("watch.ps1")).unwrap();
...
fn test_generate_watch_scripts_verbose_output() {
  let ((), out) = with_io_output(|io| {
    ...
    let deps_path = tmp.path().join("repos");
    make_repo_with_scripts(&deps_path, "user-lib1", r#""build": "tsdown""#);
    let deps = vec!["user/lib1".to_string()];
    let flags = RunFlags {
      verbose: true,
    ...
    generate_watch_scripts(tmp.path(), &deps_path, &deps, io, flags).unwrap();
  });
