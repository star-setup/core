...
fn test_resolve_workspace_errors_when_no_repos() {
  with_io_dir(|path, io| {
    create_dir_all(path.join("build-mono")).unwrap();
    let result = resolve_workspace(Some(path), None, None, io, false);
    assert!(result.is_err());
...
fn test_resolve_workspace_succeeds() {
  with_io_dir(|path, io| {
    create_dir_all(path.join("build-mono").join("repos")).unwrap();
    let result = resolve_workspace(Some(path), None, None, io, false);
    assert!(result.is_ok());
...
fn test_resolve_workspace_finds_repos() {
  with_io_dir(|path, io| {
    ...
    create_dir_all(repos.join("user-lib1").join(".git")).unwrap();
    create_dir_all(repos.join("user-lib2").join(".git")).unwrap();
...
fn test_resolve_workspace_custom_mono_dir() {
  with_io_dir(|path, io| {
    create_dir_all(path.join("my-workspace").join("repos")).unwrap();
...
fn test_resolve_workspace_excludes_non_git_dirs() {
  with_io_dir(|path, io| {
    let repos = path.join("build-mono").join("repos");
    create_dir_all(repos.join("user-lib1").join(".git")).unwrap();
    create_dir_all(repos.join("not-a-repo")).unwrap();
    let ws = resolve_workspace(Some(path), None, None, io, false).unwrap();
