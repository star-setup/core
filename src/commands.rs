...
fn print_mode_header(
  ...
  if let Some(r) = test_repo {
    println!("  Test Repository: {r}");
  } else if let Some(r) = repo_name {
    println!("  Repository: {r}");
  }
  println!("  Clone Method: {}", if use_ssh { "SSH" } else { "HTTPS" });
  if let Some(d) = mono_dir {
    println!("  Directory: {d}");
  }
  if let Some(c) = lib_count {
    println!("  Libraries: {c}");
  }
  println!();
}

fn cmake_build(args: &ResolvedArgs, build_path: &Path, mono: bool) -> Result<(), String> {
  ...
  run_command(&cmake_cmd, Some(build_path), args.connection.verbose)?;
...
pub fn single_repo_mode(args: &ResolvedArgs) -> Result<(), String> {
  ...
  cmake_build(args, build_path.as_path(), false)?;
...
pub fn mono_repo_mode(args: &ResolvedArgs, config: &SetupConfig) -> Result<(), String> {
  ...
  cmake_build(args, build_path.as_path(), true)?;
