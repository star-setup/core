...
pub fn single_repo_mode(args: &Args) -> Result<(), String> {
  ...
  let repo_url = resolve_repo_url(repo, args.ssh);
  ...
  print_mode_header("Single Repository Mode", None, Some(&repo_name), args.ssh, None, None, None);

  let repo_path = Path::new(&repo_name);
  if repo_path.exists() {
    println!("Repository {} already exists", repo_name);
    ...
    if input.trim().to_lowercase() == "y" {
      println!("Updating {}\n", repo_name);
      run_command(&["git", "pull"], Some(&repo_name), args.verbose)?;
    }
  } else {
    println!("Cloning {}\n", repo_name);
    run_command(&["git", "clone", &repo_url], None, args.verbose)?;
  }

  let build_path = PathBuf::from(&repo_name).join(&args.build_dir);
  if args.clean && build_path.exists() {
  ...
  let build_type = format!("-DCMAKE_BUILD_TYPE={}", args.build_type);
  let mut cmake_cmd = vec!["cmake", "..", &build_type];
  let cmake_args: Vec<&str> = args.cmake_args.iter().map(String::as_str).collect();
  cmake_cmd.extend(cmake_args.iter());
  run_command(&cmake_cmd, Some(build_path.to_str().unwrap()), args.verbose)?;

  if !args.no_build {
    println!("Building project\n");
    run_command(
      &["cmake", "--build", ".", "--config", &args.build_type],
      Some(build_path.to_str().unwrap()),
      args.verbose
    )?;
  }

  println!("Project finished in {}/{}", repo_name, args.build_dir);
  Ok(())
}

fn create_mono_repo_cmakelists(mono_dir: &Path, test_repo: &str, repos: &[String]) -> Result<(), String> {
  let module_names: Vec<&str> = repos.iter()
    .map(|r| r.split('/').last().unwrap_or(r.as_str()))
    .collect();
...
pub fn mono_repo_mode(args: &Args, config: &EcosystemConfig) -> Result<(), String> {
  ...
  let test_repo = if repo_input.starts_with("http") || repo_input.starts_with("git@") {
    if repo_input.contains("github.com/") || repo_input.contains("github.com:") {
      ...
      let user = parts[parts.len()-2].split(':').last().unwrap_or("");
  ...
  let test_repo_name = test_repo.split('/').last().unwrap_or(&test_repo).to_string();

  let mut repos: Vec<String> = if let Some(profile_name) = &args.profile {
    let profile_repos = config.profiles.get(profile_name)
      .ok_or_else(|| { list_profiles(config); format!("Profile '{}' not found", profile_name) })?;
    if profile_repos.is_empty() {
      return Err(format!("Profile '{}' has no repositories", profile_name));
    }
    print_mode_header("Profile", Some(&test_repo), None, args.ssh, Some(&args.mono_dir), Some(profile_name), Some(profile_repos.len()));
    profile_repos.clone()
  } else if let Some(r) = &args.repos {
    print_mode_header("Mono-repository", Some(&test_repo), None, args.ssh, Some(&args.mono_dir), None, Some(r.len()));
    r.clone()
  } else {
  ...
  let mono_repo_path = PathBuf::from(&args.mono_dir);
  println!("Creating directory: {}\n", mono_repo_path.display());
  fs::create_dir_all(&mono_repo_path).map_err(|e| e.to_string())?;

  println!("Cloning repositories");
  for repo in &repos { clone_repository(repo, &mono_repo_path, args.ssh, args.verbose)?; }
